...
function UpdatePuppetOccupyStatus(city, player, isForced)
	...
	if isPuppet then
		city:SetNumRealBuilding(happinessModID, 1)--Game.Round(city:GetPopulation() * Civup.PUPPET_UNHAPPINESS_MOD / 100))
...
function LiberatedMinor(city, minorCiv, isForced)
	...
	for majorCivID,majorCiv in pairs(Players) do
		if majorCiv:IsAliveCiv() and not majorCiv:IsMinorCiv() and (majorCivID ~= Game.GetActivePlayer()) and (minorCiv:GetMinorCivFriendshipWithMajor(majorCivID) > Civup.MINOR_CIV_LIBERATED_FRIENDSHIP_MAX) then
			minorCiv:SetFriendship(majorCivID, Civup.MINOR_CIV_LIBERATED_FRIENDSHIP_MAX)
		end
	end
end
...
function BuildingCreated(player, city, buildingID)
	...
	if Game.HasValue( {BuildingType=buildingInfo.Type, YieldType='YIELD_CULTURE'}, GameInfo.Building_SeaPlotYieldChanges ) then
		...
		for _,adjPlot in pairs(nearbyPlots) do
			local adjPlotID = Plot_GetID(adjPlot)
			if adjPlot:GetOwner() == playerID and not MapModData.Civup.HasTerrainCulture[adjPlotID][buildingInfo.Type] then
				for row in GameInfo.Building_ResourceYieldChanges(query) do
					Plot_ChangeYield(adjPlot, YieldTypes.YIELD_CULTURE, row.Yield)
					MapModData.Civup.HasTerrainCulture[adjPlotID][row.BuildingType] = true
					SaveValue(true, "MapModData.Civup.HasTerrainCulture[%s][%s]", adjPlotID, row.BuildingType)
				end
			end
		end
	end
	...
	if Game.HasValue( {BuildingType=buildingInfo.Type, YieldType='YIELD_CULTURE'}, GameInfo.Building_ResourceYieldChanges ) then
		...
		for _,adjPlot in pairs(nearbyPlots) do
			...
			if adjResource ~= -1 and adjPlot:GetOwner() == playerID and not MapModData.Civup.HasResourceCulture[adjPlotID][buildingInfo.Type] then
				adjResource = GameInfo.Resources[adjResource].Type
				for row in GameInfo.Building_ResourceYieldChanges(query) do
					if adjResource == row.ResourceType then
						Plot_ChangeYield(adjPlot, YieldTypes.YIELD_CULTURE, row.Yield)
						MapModData.Civup.HasResourceCulture[adjPlotID][row.BuildingType] = true
						SaveValue(true, "MapModData.Civup.HasResourceCulture[%s][%s]", adjPlotID, row.BuildingType)
					end
				end
			end
		end
...
--
MapModData.Civup.HasTerrainCulture = {}
MapModData.Civup.HasResourceCulture = {}
startClockTime = os.clock()
for plotID = 0, Map.GetNumPlots() - 1, 1 do
	MapModData.Civup.HasTerrainCulture [plotID] = {}
	MapModData.Civup.HasResourceCulture[plotID] = {}
	local plot = Map.GetPlotByIndex(plotID)
	if plot:GetOwner() ~= -1 then
		--[[
		for row in GameInfo.Building_SeaPlotYieldChanges ("YieldType = 'YIELD_CULTURE'") do
			MapModData.Civup.HasTerrainCulture[plotID][row.BuildingType] = LoadValue("MapModData.Civup.HasTerrainCulture[%s][%s]", plotID, row.BuildingType)
		end
		for row in GameInfo.Building_ResourceYieldChanges("YieldType = 'YIELD_CULTURE'") do
			MapModData.Civup.HasResourceCulture[plotID][row.BuildingType] = LoadValue("MapModData.Civup.HasResourceCulture[%s][%s]", plotID, row.BuildingType)
		end
		--]]
	end
end
...
function OnPlotAcquired(plot, playerID)
	...
	if city then
		...
		for row in GameInfo.Building_SeaPlotYieldChanges("YieldType = 'YIELD_CULTURE'") do
			...
			if not MapModData.Civup.HasTerrainCulture[plotID][row.BuildingType] then
				if city:IsHasBuilding(buildingID) then
					--log:Debug("HasTerrainCulture")
					Plot_ChangeYield(plot, YieldTypes.YIELD_CULTURE, row.Yield)
					MapModData.Civup.HasTerrainCulture[plotID][row.BuildingType] = true
					SaveValue(true, "MapModData.Civup.HasTerrainCulture[%s][%s]", plotID, row.BuildingType)
				end
			end
		end
		for row in GameInfo.Building_ResourceYieldChanges("YieldType = 'YIELD_CULTURE'") do
			local buildingID = GameInfo.Buildings[row.BuildingType].ID
			if not MapModData.Civup.HasResourceCulture[plotID][row.BuildingType] then
				if city:IsHasBuilding(buildingID) and (plot:GetResourceType(-1) == GameInfo.Resources[row.ResourceType].ID) then
					...
					Plot_ChangeYield(plot, YieldTypes.YIELD_CULTURE, row.Yield)
					MapModData.Civup.HasResourceCulture[plotID][row.BuildingType] = true
					SaveValue(true, "MapModData.Civup.HasResourceCulture[%s][%s]", plotID, row.BuildingType)
				end
			end
		end
		--]]
...
function BuildingLost(player, city, buildingID)
	...
	if Game.HasValue( {BuildingType=buildingInfo.Type, YieldType='YIELD_CULTURE'}, GameInfo.Building_ResourceYieldChanges ) then
		...
		for _,adjPlot in pairs(nearbyPlots) do
			local adjPlotID = Plot_GetID(adjPlot)
			local adjResource = adjPlot:GetResourceType(-1)
			if adjResource ~= -1 and adjPlot:GetOwner() == playerID and MapModData.Civup.HasResourceCulture[adjPlotID][buildingInfo.Type] then
				adjResource = GameInfo.Resources[adjResource].Type
				for row in GameInfo.Building_ResourceYieldChanges(query) do
					if adjResource == row.ResourceType then
						Plot_ChangeYield(adjPlot, YieldTypes.YIELD_CULTURE, -row.Yield)
						MapModData.Civup.HasResourceCulture[adjPlotID][row.BuildingType] = false
						SaveValue(true, "MapModData.Civup.HasTerrainHasResourceCultureCulture[%s][%s]", adjPlotID, row.BuildingType)
					end
				end
			end
		end
	...
	if Game.HasValue( {BuildingType=buildingInfo.Type, YieldType='YIELD_CULTURE'}, GameInfo.Building_SeaPlotYieldChanges ) then
		...
		for _,adjPlot in pairs(nearbyPlots) do
			local adjPlotID = Plot_GetID(adjPlot)
			if adjPlot:GetOwner() == playerID and MapModData.Civup.HasTerrainCulture[adjPlotID][buildingInfo.Type] then
				for row in GameInfo.Building_ResourceYieldChanges(query) do
					Plot_ChangeYield(adjPlot, YieldTypes.YIELD_CULTURE, -row.Yield)
					MapModData.Civup.HasTerrainCulture[adjPlotID][row.BuildingType] = false
					SaveValue(true, "MapModData.Civup.HasTerrainCulture[%s][%s]", adjPlotID, row.BuildingType)
				end
			end
		end
...
function DoPolicyFreeBuildingClasses(player, policyID)
	...
	for row in GameInfo.Policy_FreeBuildingClass() do
		...
		if (policyID == testPolicyInfo.ID) or player:HasPolicy(testPolicyInfo.ID) then
			...
			if not MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] then
				MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] = 0
			end
			if numCities == -1 then
				for city in player:Cities() do
			...
			elseif (MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] < numCities) then
		...
		if (policyID == testPolicyInfo.ID) or player:HasPolicy(testPolicyInfo.ID) then
			...
			if numCities == -1 then
				for city in player:Cities() do
					...
					if not MapModData.Civup.FreeFlavorBuilding[flavorType][cityID] then
						PlaceBuildingOfFlavor(city, flavorType)
					end
				end
			elseif (MapModData.Civup.PlayerFreeBuildings.Flavors[flavorType][playerID] < numCities) then
				if not cityList then
					cityList = {}
					for city in player:Cities() do
						if player:IsHuman() then
				...
				for i = 1, maxCityID do
					local cityID = cityList[i].id
					if player:IsHuman() then
						log:Debug("cityID=%s freeBuilding=%s", cityID, MapModData.Civup.FreeFlavorBuilding[flavorType][cityID])
					end
					local city = Map_GetCity(cityID)
					if not MapModData.Civup.FreeFlavorBuilding[flavorType][cityID] then
						PlaceBuildingOfFlavor(city, flavorType)
...
if not MapModData.Civup.FreeFlavorBuilding then
	MapModData.Civup.FreeBuilding = {}
	MapModData.Civup.FreeFlavorBuilding = {}
	MapModData.Civup.PlayerFreeBuildings = {}
	MapModData.Civup.PlayerFreeBuildings.Buildings = {}
	MapModData.Civup.PlayerFreeBuildings.Flavors = {}
	startClockTime = os.clock()
	for playerID,player in pairs(Players) do
		MapModData.Civup.PlayerFreeBuildings.Buildings[playerID] = {}
		if UI:IsLoadedGame() then
			for city in player:Cities() do
				local cityID = City_GetID(city)			
				MapModData.Civup.FreeBuilding[cityID] = {}
				local buildingID = LoadValue("MapModData.Civup.FreeBuilding[%s]", cityID)
				--log:Trace("Loading %15s MapModData.Civup.FreeBuilding %s = %s", city:GetName(), flavorType, buildingID)
				if buildingID then
					MapModData.Civup.FreeBuilding[cityID][buildingID] = true
					if not MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] then
						MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] = 1
					else
						MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] = MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] + 1
					end
				end
			end
		end
	...
	for row in GameInfo.Flavors() do
		...
		MapModData.Civup.FreeFlavorBuilding[flavorType] = {}
		MapModData.Civup.PlayerFreeBuildings.Flavors[flavorType] = {}
		for playerID,player in pairs(Players) do
			MapModData.Civup.PlayerFreeBuildings.Flavors[flavorType][playerID] = 0
			if UI:IsLoadedGame() then
				for city in player:Cities() do
					local cityID = City_GetID(city)
					local buildingID = LoadValue("MapModData.Civup.FreeFlavorBuilding[%s][%s]", flavorType, cityID)
					--log:Trace("Loading %15s MapModData.Civup.FreeFlavorBuilding %s = %s", city:GetName(), flavorType, buildingID)
					if buildingID then
						MapModData.Civup.FreeFlavorBuilding[flavorType][cityID] = buildingID
						MapModData.Civup.PlayerFreeBuildings.Flavors[flavorType][playerID] = MapModData.Civup.PlayerFreeBuildings.Flavors[flavorType][playerID] + 1
					end
				end
			end
		end
...
function PlaceBuildingID(city, buildingID)
	...
	local player = Players[playerID]
	local cityID = City_GetID(city)
	if not MapModData.Civup.FreeBuilding[cityID] then
		MapModData.Civup.FreeBuilding[cityID] = {}
	end
	if MapModData.Civup.FreeBuilding[cityID][buildingID] then
		return
	end
	city:SetNumRealBuilding(buildingID, 1)
	SaveValue(buildingID, "MapModData.Civup.FreeBuilding[%s]", cityID)
	MapModData.Civup.FreeBuilding[cityID][buildingID] = true
	MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] = MapModData.Civup.PlayerFreeBuildings.Buildings[playerID][buildingID] + 1
	if player:IsHuman() then
...
function PlaceBuildingOfFlavor(city, flavorType)
	...
	if buildingID ~= -1 then
		city:SetNumRealBuilding(buildingID, 1)
		local cityID = City_GetID(city)
		SaveValue(buildingID, "MapModData.Civup.FreeFlavorBuilding[%s][%s]", flavorType, cityID)
		MapModData.Civup.FreeFlavorBuilding[flavorType][cityID] = buildingID
		MapModData.Civup.PlayerFreeBuildings.Flavors[flavorType][playerID] = MapModData.Civup.PlayerFreeBuildings.Flavors[flavorType][playerID] + 1
		if player:IsHuman() then
	...
	if exp > 0 then
		for unit in player:Units() do
			if unit and not unit:IsDead() and unit:IsGarrisoned() then
				unitXP = (MapModData.Civup.UnitXP[playerID][unit:GetID()] or 0) + exp
				if unitXP >= 1 then
					unit:ChangeExperience(math.floor(unitXP))
					unitXP = unitXP - math.floor(unitXP)
				end
				MapModData.Civup.UnitXP[playerID][unit:GetID()] = unitXP
				SaveValue(unitXP, "MapModData.Civup.UnitXP[%s][%s]", playerID, unit:GetID())
...
if not MapModData.Civup.UnitLevel then
	MapModData.Civup.UnitLevel = {}
	for playerID,player in pairs(Players) do
		if player:IsAliveCiv() then
			MapModData.Civup.UnitLevel[playerID] = {}
			for unit in player:Units() do
				MapModData.Civup.UnitLevel[playerID][unit:GetID()] = unit:GetLevel()
			end
		end
	end
end
...
function DoPromotionHeal(unit, promotionType)
	...
	if heal and heal ~= 0 then
		...
		MapModData.Civup.UnitLevel[playerID][unit:GetID()] = (MapModData.Civup.UnitLevel[playerID][unit:GetID()] or unit:GetLevel()) + 1
	end
end
...
function CheckForAILevelup(unit)
	local playerID = unit:GetOwner()
	MapModData.Civup.UnitLevel[playerID][unit:GetID()] = MapModData.Civup.UnitLevel[playerID][unit:GetID()] or unit:GetLevel()
	for i = MapModData.Civup.UnitLevel[playerID][unit:GetID()], unit:GetLevel() - 1 do
		--log:Warn("%15s %15s old=%s new=%s", Players[playerID]:GetName(), unit:GetName(), MapModData.Civup.UnitLevel[playerID][unit:GetID()], unit:GetLevel())
