...
function AddBuildingButton( city, building )
	...
	if(building.GreatWorkSlotType ~= nil and building.GreatWorkCount > 0) then
		...
		local themeBonus = city:GetThemingBonus(iBuildingClass)
		if(themeBonus > 0) then
			local themeBonusToolTip = city:GetThemingTooltip(iBuildingClass)
		...
		for i = 0, iNumGreatWorks - 1, 1 do
			...
			local iGreatWorkIndex = city:GetBuildingGreatWork(iBuildingClass, i)
			if (iGreatWorkIndex >= 0) then
				filledGreatWorkSlot:SetHide(false)
				filledGreatWorkSlot:SetTexture(filledTexture)
				filledGreatWorkSlot:SetToolTipString(Game.GetGreatWorkTooltip(iGreatWorkIndex, city:GetOwner()))
